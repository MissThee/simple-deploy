...
export default async (opts) => {
    ...
    const envKeys = opts.environment;
    processWatcher(() => {
        // 进程退出前清理临时文件目录
    ...
    try {
        ...
        for (const currentEnvKey of envKeys) {
            ...
            // 测试连接ssh
            await deployTool.sshCheck(currentEnv.server.serverHost, currentEnv.server.serverPort, currentEnv.server.serverUsername, configFile.local.sshPrivateKeyPath, configFile.local.sshPassphrase, currentEnv.server.serverPassword);
        }
        let isFirstEnv = true;
        ...
        for (const currentEnvKey of envKeys) {
            ...
            // 连接ssh
            const ssh = await deployTool.sshConnect(currentEnv.server.serverHost, currentEnv.server.serverPort, currentEnv.server.serverUsername, configFile.local.sshPrivateKeyPath, configFile.local.sshPassphrase, currentEnv.server.serverPassword);
