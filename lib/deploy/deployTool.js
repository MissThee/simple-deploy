...
// 确认是否部署提示
export const confirmDeploy = (param, envs) => {
...
// 检查配置文件
export const getCorrectConfigFile = async (configFilePath, envKeys) => {
...
// 执行打包脚本
export const buildCode = async (script, isVerbMode) => {
    if (isVerbMode) {
...
export const getLocalArchiveFilePathByProjectPath = (projectPath) => {
    return path.join(process.cwd(), deployLocalTmpPath, path.basename(projectPath) + '_deploy.tar.gz');
};
// 获取远程归档文件全路径
export const getRemoteArchiveFilePath = async (projectPath, remotePath) => {
...
// 创建临时存储目录
export const createDir = async (filePath) => {
...
// 归档
export const buildArchive = async (sourcePath, outputFile) => {
...
// 删除本地文件
export const removeFile = async (message, ...localPaths) => {
...
// 删除本地文件，同步，线程退出时清理使用
export const removeFileSync = (message, ...localPaths) => {
    let isFileExist = false;
    for (const localPath of localPaths) {
        if (fsExtra.existsSync(localPath)) {
            isFileExist = true;
...
// 测试ssh连通
export const sshCheck = async (host, port, username, privateKey, passphrase, password) => {
...
// 连接ssh
export const sshConnect = async (host, port, username, privateKey, passphrase, password) => {
    if (privateKey && privateKey.trimStart().startsWith('~')) {
        privateKey = path.join(os.homedir(), privateKey.substring(privateKey.indexOf('~') + 1));
    }
    let sshConfig = {
...
// 上传文件
export const sshUploadFile = async (ssh, localArchiveFile, remoteFile) => {
    ss.start('REMOTE Upload File', ' ', chalk.magenta(localArchiveFile));
    await ssh.putFile(path.normalize(localArchiveFile), path.normalize(remoteFile), null, {
    // concurrency: 64,
    // chunkSize:32768,
    });
...
// 删除远程文件
export const sshRemoveFile = async (ssh, ...remotePaths) => {
...
// 解压远程文件
export const sshUnpackFile = async (ssh, ...remoteFiles) => {
...
// 远程单文件改名
const sshRenameFile = async (ssh, remoteFile, newName) => {
...
//远程文件改名
export const sshRenameFileByFullPath = async (ssh, projectPath, remotePath) => {
    if ((await fsExtra.stat(path.join(process.cwd(), projectPath))).isFile() && !remotePath.replace(/\\/g, '/').endsWith('/')) {
        //使用远程文件全路径 和 本地文件全路径截取的最后一部分文件名拼接，找到未改名称的远程文件
        const remoteFileBeforeRename = path.join(remotePath.substring(0, remotePath.lastIndexOf(path.basename(remotePath))), path.basename(projectPath));
...
// 断开ssh
export const sshDisconnect = (ssh) => {
